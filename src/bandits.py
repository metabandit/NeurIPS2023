...
bandit
X_order_bandit
INPUT: P_intervals =[(0, .1), (0.2, 0.3), (0.7, 0.8), (0.9, 1.)] or a single P
In case of P_intervals, a P is drawn through draw_from_intervals(intervals) each time reset() is called.
OUTPUT:
reward: 0 or 1; 0 - Left, 1 - Right
done: False or True; True if timestep >= maxtimestep
timestep:  
correct: 1 or 0; 1 for choosing the arm with the highest reward probability
classes for bandits
X_order_bandit is generated with either P or P_intervals. If P is set, then this bandit will always have fixed probability P of correspondent’s order switch. If P is not fixed, then probability is drawn from P_intervals each time bandit reset is called.
During meta learning, reset is called each time episode finishes
reset() – resets bandit, draws new P from P_interval if P is not fixed
update() – updates timestep and reward’s position
act() – receives action, calls update, returns reward and information about trial

bandit = first_order_bandit()

bandit.reset()
print(bandit.P)
for i in range(bandit.maxtimestep):
    reward, done, t, correct = bandit.act(0)
    print(f'reward is {reward}, d is {done}, timestep {t}, correct {correct}')
    
print(type(bandit.name))    
"""

def draw_from_intervals(intervals):
    #rng = np.random.default_rng()
    chosen_interval = intervals[rng.integers(len(intervals))]
    res = rng.uniform(*chosen_interval)
    return res
...
class zero_order_bandit():
    ...
    def reset(self, P=None):
        self.prev_reward_pos = rng.choice(self.actions,)
        self.prev_prev_reward_pos = rng.choice(self.actions,)
        if P is not None:
            self.P = P
            self.fix_P = True
        else:
        ...
        new_reward_pos = rng.choice(self.actions, p=[self.P, 1 - self.P])
        self.reward = [0, 0]
    ...
    def update(self):
        ...
        self.reward_pos = rng.choice(self.actions, p = [self.P, 1- self.P])
    ...
    def act(self, action):
        ...
        return reward, done, self.timestep, correct
...
class first_order_bandit():
    ...
    def reset(self, P = None):
        ...
        reward_pos = rng.choice(self.actions,)
        self.reward = [0, 0]
